        ...
        if isinstance(event, events.TrackingEvent):
            for hand in event.hands:
                if hand.type == enums.HandType.Right:
                    ...
                    # Mapear cantidad a letra
                    conteo_a_letra = {
                        1: "a",  # solo un dedo extendido
                        2: "b",
                        3: "c",
                        4: "d",
                        5: "e"
                    ...
                    letra = conteo_a_letra.get(total_dedos, "x")  # "x" si no es un número válido

                    if letra != self.ultimo_envio:
                        print(f"✋ Dedos extendidos: {total_dedos} → Código: {letra}")
...
def main():
    ...
    with conn.open():
        print("🟢 Reconociendo conteo de dedos... (Ctrl+C para salir)")
        try:
            while True:
                time.sleep(0.05)
